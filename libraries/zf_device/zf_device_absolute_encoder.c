...
static int16 now_location[ABSOLUTE_ENCODER_CS_PIN_MAX] = {0};
static int16 last_location[ABSOLUTE_ENCODER_CS_PIN_MAX] = {0};

gpio_pin_enum absolute_encoder_cs_list[ABSOLUTE_ENCODER_CS_PIN_MAX] = {ENCODER_CS_PIN_LIST};
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     绝对值编码器获取相较上次位置的偏移值（修正版本）
// 参数说明     index           编码器索引
// 返回参数     int16           偏移值
// 使用示例     absolute_encoder_get_offset_fix(0);
// 备注信息     处理跨越零点时的增量值修正，避免跳变
//-------------------------------------------------------------------------------------------------------------------
int16 absolute_encoder_get_offset_fix(uint8 index)
{
    int16 delta_p = now_location[index] - last_location[index];

    // 如果位置穿越了零点，我们简单的认为他发生了跨圈数，我们需要还原这个增量值
    // 编码器范围是 0-4095，中点是 2048
    // 当差值超过半圈（2048）时，说明发生了跨零点，需要修正
    if (delta_p > 2048)
    {
        // 从小数值跳到大数值（逆时针跨零点），实际应该是负方向移动
        delta_p -= 4096;
    }
    else if (delta_p < -2048)
    {
        // 从大数值跳到小数值（顺时针跨零点），实际应该是正方向移动
        delta_p += 4096;
    }
